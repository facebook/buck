...
bazel build //src/main/java/com/google/devtools/build/lib:bazel_deploy.jar \
  //src/main/java/com/google/devtools/build/lib:bazel_deploy-src.jar
java -jar "${JARJAR_PATH}" process "${MY_DIR}/jarjar-rules.txt" \
  bazel-bin/src/main/java/com/google/devtools/build/lib/bazel_deploy.jar \
  "${MY_DIR}/${FINAL_JAR_NAME}_deploy.jar"
java -jar "${JARJAR_PATH}" process "${MY_DIR}/jarjar-rules.txt" \
  bazel-bin/src/main/java/com/google/devtools/build/lib/bazel_deploy-src.jar \
