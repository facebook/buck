...
public final class Label
  ...
  public Label getRelative(String relName, StarlarkThread thread) throws LabelSyntaxException {
    // Bazel uses a BazelStarlarkContext in order to get the repo mapping. At this time, we
    // do not support this remapping and do not provide a BazelStarlarkContext, so remove its
    // invocation. Also, this is the only usage of BazelStarlarkContext, so we remove that dep.
    return getRelativeWithRemapping(relName, ImmutableMap.of());
  }
