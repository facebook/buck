...
files_to_update='.classpath build.xml programs/classpaths .idea/libraries/buck_lib.xml third-party/java/thrift/BUCK'
for f in $files_to_update; do
  sudo sed -i '' "s/libthrift-$old/libthrift-$version/g" $f
done
