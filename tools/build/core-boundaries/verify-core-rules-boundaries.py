...
def get_actual_core_targets(repo_root):
    ...
    cmd_output = run_process(cmd_prefix + ['query', CORE_QUERY, '--json'], env=env, cwd=repo_root)
...
def verify(repo_root):
    ...
    print("The transitive closure of Buck core dependencies is inconsistent with allowed "
          "set of targets.")
    print("Try to refactor the dependencies to keep the set of core targets minimal.")
    print("In case an existing dependency was removed from the core remove that target from "
