 ...
 * dependencies.
 * <p/>
 * In order to be compatible, type references to missing classes in the non-private interface of
 * a class being compiled must be one of the following:
 * <ul>
 * <li>The fully-qualified name of the type</li>
 * <li>The simple name of a type that appears in an import statement</li>
 * <li>The simple name of a top-level type in the same package as the type being compiled</li>
 * </ul>
 * <p/>
 * In order to be compatible, compile-time constants in missing classes may not be referenced from
 * the non-private interface of a class being compiled.
 */
      ...
      new InterfaceTypeAndConstantReferenceFinder(
          ...
          new InterfaceTypeAndConstantReferenceFinder.Listener() {
            ...
            public void onTypeReferenceFound(
              ...
              String minimalQualifiedName = findMinimalQualifiedName(
                  path,
                  enclosingPackage,
                  referencedType);
            ...
            public void onConstantReferenceFound(
              ...
              trees.printMessage(
                  messageKind,
                  String.format("Must inline the constant value: %s", constant.getConstantValue()),
                  path.getLeaf(),
                  path.getCompilationUnit());
            }
            ...
            private boolean isFullyQualified(TreePath path, TypeElement referencedTypeElement) {
              return referencedTypeElement.getQualifiedName()
            ...
            private String findMinimalQualifiedName(
              ...
              while (walker.getKind() != ElementKind.PACKAGE &&
                  !referenceIsLegalForMissingTypes(path, enclosingPackage, (TypeElement) walker)) {
