...
public class CGoGenSource extends AbstractBuildRule {
  ...
  private final ImmutableSet<String> includeDirs;
  ...
  public CGoGenSource(
    ...
    Builder<String> includeBuilder = ImmutableSortedSet.naturalOrder();
    for (Path path : headers.keySet()) {
      SourcePath srcPath = headers.get(path);
      if (srcPath instanceof BuildTargetSourcePath
          && ((BuildTargetSourcePath) srcPath)
              .getTarget()
              .getBasePath()
              .equals(buildTarget.getBasePath())) {
        // allow generated header files from the same base path to be included by cgo files without
        // using full path. This is needed when the whole cgo package is generated by other Buck
        // rules
        if (Files.isDirectory(path)) {
          includeBuilder.add(pathResolver.getRelativePath(srcPath).toString());
        } else {
          includeBuilder.add(pathResolver.getRelativePath(srcPath).getParent().toString());
        }
      }
    }
    includeDirs = includeBuilder.build();
  }
