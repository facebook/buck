...
public class GoTestCoverSource extends AbstractBuildRule {
  ...
  public GoTestCoverSource(
    ...
    this.genDir = BuildTargetPaths.getGenPath(projectFilesystem.getBuckPaths(), buildTarget, "%s");
    this.buildDeps =
  ...
  static String getVarName(Path path) {
    return "Var_"
        + Hashing.sha256().hashString(path.getFileName().toString(), StandardCharsets.UTF_8);
  }
