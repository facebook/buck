...
public class ThriftCxxEnhancer implements ThriftLanguageSpecificEnhancer {
  ...
  private CxxHeadersAndSources getThriftHeaderSourceSpec(
    ...
    for (ImmutableMap.Entry<String, ThriftSource> ent : sources.entrySet()) {
      ...
      final Path outputDir = source.getOutputDir(ruleResolver);

      for (String partialName :
           getGeneratedSources(target, args, thriftName, source.getServices())) {
  ...
  private ImmutableSortedSet<BuildRule> getAllDeps(
  ...
  private CxxLibraryDescription.Arg createLangArg(
      BuildTarget target,
      BuildRuleResolver resolver,
      ThriftConstructorArg args,
      ImmutableMap<String, ThriftSource> sources,
    ...
    CxxHeadersAndSources spec = getThriftHeaderSourceSpec(target, resolver, args, sources);

    // Add all the passed in language-specific thrift deps, and any C/C++ specific deps
    // passed in via the constructor arg.
    ImmutableSortedSet<BuildRule> allDeps = getAllDeps(resolver, args, deps);
  ...
  public BuildRule createBuildRule(
    ...
    // Create the language-specific constructor arg.
    CxxLibraryDescription.Arg langArgs =
        createLangArg(
  ...
  public <U> Optional<U> createMetadata(
      ...
      throws NoSuchBuildTargetException {
    return cxxLibraryDescription.createMetadata(
        buildTarget,
        resolver,
        createLangArg(
            buildTarget,
