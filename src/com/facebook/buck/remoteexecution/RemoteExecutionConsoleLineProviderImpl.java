...
public class RemoteExecutionConsoleLineProviderImpl implements RemoteExecutionConsoleLineProvider {
  ...
  public RemoteExecutionConsoleLineProviderImpl(
  ...
  public ImmutableList<String> createConsoleLinesAtTime(long currentTimeMillis) {
    ...
    if (showCasStatsLine) {
      String casLine =
          String.format(
              "RE CAS: Upl=[Count:%d Bytes=%s] Dwl=[Count:%d Bytes=%s]",
  ...
  private int getLocallyBuiltRules(
    ...
    int remotelyExecutedBuildRules =
        actionsPerState.get(State.ACTION_SUCCEEDED) + actionsPerState.get(State.ACTION_FAILED);
