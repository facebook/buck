...
interface KeyedLocker<K> {
  ...
  interface AutoUnlocker extends AutoCloseable {
    ...
    /**
     * Closes the {@link AutoUnlocker} instance. If this instance was the last unclosed one returned
     * by {@link #writeLock} with argument {@code k} owned by the current thread, then exclusive
     * access to {@code k} is yielded. If this instance was the last unclosed one returned by {@link
     * #readLock} with argument {@code k}, then a thread can request exclusive write access using
     * {@link #writeLock} with argument {@code k}.
     *
     * <p>This method may only be called at most once per {@link AutoUnlocker} instance and must be
     * called by the same thread that acquired the {@link AutoUnlocker} via {@link #writeLock} or
     * {@link #readLock}. Otherwise, an {@link IllegalUnlockException} is thrown.
     */
    @Override
    void close();
  }
  ...
  /**
   * Blocks the current thread until it has exclusive access to do things with {@code k} and returns
   * a {@link AutoUnlocker} instance for yielding the implicit lock.
   *
