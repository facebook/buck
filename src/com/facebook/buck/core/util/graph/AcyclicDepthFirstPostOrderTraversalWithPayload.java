...
public class AcyclicDepthFirstPostOrderTraversalWithPayload<T, P> {
  ...
  private final GraphTraversableWithPayload<T, P> traversable;
  ...
  public LinkedHashMap<T, P> traverse(Iterable<? extends T> initialNodes) throws CycleException {
  ...
  public LinkedHashMap<T, P> traverse(
    ...
    LinkedHashMap<T, P> explored = new LinkedHashMap<>();
    ...
    while (!toExplore.isEmpty()) {
      ...
      if (shouldExploreChildren.test(node)) {
        for (Iterator<? extends T> iter = explorable.children; iter.hasNext(); ) {
          ...
          } else if (!explored.containsKey(child)) {
      ...
      if (stackSize == toExplore.size()) {
        ...
        explored.put(node, explorable.payload);
      }
    }
  ...
  private class Explorable {
    ...
    Explorable(T node) {
      this.node = node;
      Pair<P, Iterator<? extends T>> x = traversable.findNodeAndChildren(node);
