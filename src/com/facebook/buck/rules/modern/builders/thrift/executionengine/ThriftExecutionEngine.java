...
public class ThriftExecutionEngine implements RemoteExecutionService {
  ...
  public ThriftExecutionEngine(
    ...
    // TODO(orr): Temp just to please the linter, will implement in next diff.
  ...
  public ExecutionResult execute(Protocol.Digest actionDigest)
    ...
    //      synchronized (client) {
    //        // TODO(shivanker): Thrift client is not thread-safe. We *really* don't want this for
    // the
    //        // long-term. Will convert this to an async-client in the next diff.
    //        response = client.execute(request);
    //      }
    //      com.facebook.remoteexecution.executionengine.ExecutionResult result =
    // waitForResult(response);
  ...
  //
  //      GetExecutionStateRequest request = new
  // GetExecutionStateRequest(response.state.execution_id);
