 ...
 */
public class MergeAssetsExternalAction implements ExternalAction {
  ...
  public ImmutableList<IsolatedStep> getSteps(BuildableCommand buildableCommand) {
    ...
    MergeAssetsExternalActionArgs args =
        ExternalActionsUtils.readJsonArgs(
            Iterables.getOnlyElement(json), MergeAssetsExternalActionArgs.class);

    return ImmutableList.of(
        new MergeAssetsStep(
            RelPath.get(args.getPathToMergedAssets()),
            args.getPathToBaseApk().map(path -> RelPath.get(path)),
            args.getAssetsDirectories().stream()
                .map(RelPath::get)
