...
/**
 * A {@link com.facebook.buck.shell.Command} that creates or updates a ZIP archive using {@code zip}.
 *
 * @see <a href="http://www.info-zip.org/mans/zip.html">ZIP</a>
 */
public class ZipCommand extends ShellCommand {
  ...
  /**
   * Create a {@link ZipCommand} to create or update a zip archive.
   *
   ...
   *
   * @param mode one of {@link ZipCommand.Mode#ADD}, {@link ZipCommand.Mode#UPDATE UPDATE},
   *    {@link ZipCommand.Mode#FRESHEN FRESHEN} or {@link ZipCommand.Mode#DELETE DELETE}, as in the
   *    {@code zip} command.
  ...
  /**
   * Create a {@link ZipCommand} that adds an entire directory to an archive. The files directly in
   ...
   *
   * @see #ZipCommand(Mode, String, Set, boolean, int, File)
   */
  public ZipCommand(String pathToZipFile, File directoryToAdd) {
    this(
        Mode.ADD,
        pathToZipFile,
