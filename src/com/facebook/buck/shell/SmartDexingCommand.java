 ...
 */
public class SmartDexingCommand implements Command {
  ...
  private void runDxCommands(ExecutionContext context, Multimap<File, File> outputToInputs)
    ...
    DefaultCommandRunner commandRunner = new DefaultCommandRunner(context, getDxExecutor());
    ...
    List<Command> dxCommands = generateDxCommands(context, outputToInputs);
    commandRunner.runCommandsInParallelAndWait(dxCommands);
  }
   ...
   * to a {@code .dex} file.
   */
  private List<Command> generateDxCommands(
  ...
  static class DxPseudoRule {
    ...
    public List<Command> buildInternal() {
      Preconditions.checkState(newInputsHash != null, "Must call checkIsCached first!");

      final DxCommand dxCommand = new DxCommand(outputPath, srcs);
      final WriteFileCommand updateHashCommand = new WriteFileCommand(
      ...
      return ImmutableList.<Command>of(new CompositeCommand(
