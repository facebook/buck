...
public class AaptPackageResources extends AbstractBuildRule {
  ...
  private RelPath getPathToRDotTxtDir() {
  ...
  public ImmutableList<Step> getBuildSteps(
    ...
    steps.add(
        MkdirStep.of(
            BuildCellRelativePath.fromCellRelativePath(
                ...
                getAndroidManifestXml().getParent())));
    steps.add(
        CopyStep.forFile(
    ...
    RelPath rDotTxtDir = getPathToRDotTxtDir();
    ...
    RelPath pathToGeneratedProguardConfig = getPathToGeneratedProguardConfigFile();
    ...
    buildableContext.recordArtifact(pathToGeneratedProguardConfig.getPath());
    ...
    buildableContext.recordArtifact(getResourceApkPath().getPath());
   ...
   * #manifest}.
   */
  private RelPath getAndroidManifestXml() {
    return BuildTargetPaths.getScratchPath(
        getProjectFilesystem(), getBuildTarget(), "__manifest_%s__/AndroidManifest.xml");
  }

  /** @return Path to the unsigned APK generated by this {@link BuildRule}. */
  private RelPath getResourceApkPath() {
    return BuildTargetPaths.getGenPath(
        getProjectFilesystem().getBuckPaths(), getBuildTarget(), RESOURCE_APK_PATH_FORMAT);
  }

  private RelPath getPathToGeneratedProguardConfigFile() {
    return BuildTargetPaths.getGenPath(
        getProjectFilesystem().getBuckPaths(), getBuildTarget(), "%s/proguard/proguard.txt");
  }
