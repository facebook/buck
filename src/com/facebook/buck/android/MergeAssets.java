...
public class MergeAssets extends AbstractBuildRule {
  ...
  public ImmutableList<Step> getBuildSteps(
    ...
    // TODO(cjhopman): Once this has been in use for a while and nobody depends on aapt's deprecated
    // handling of .gz files, delete this check and allow .gz files to be added directly.
    steps.add(new AbstractExecutionStep("check_gz_assets") {
      @Override
      public StepExecutionResult execute(ExecutionContext context)
          throws IOException, InterruptedException {
        getProjectFilesystem()
            .getFilesUnderPath(getScratchAssetsDirectory())
            .forEach(p -> Preconditions.checkArgument(
                    !Files.getFileExtension(p.toString()).equals("gz"),
