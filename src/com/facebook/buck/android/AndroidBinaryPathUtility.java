...
public final class AndroidBinaryPathUtility {
  ...
  static Path getZipalignedApkPath(
    ...
    return Paths.get(
        getUnsignedApkPath(projectFilesystem, buildTarget, binaryType)
            ...
            // TODO: This was zipaligned before
   ...
   */
  static RelPath getProguardTextFilesPath(
      ProjectFilesystem projectFilesystem, BuildTarget buildTarget) {
    return BuildTargetPaths.getGenPath(
        projectFilesystem.getBuckPaths(), buildTarget, "%s/proguard");
  }

  /** The APK at this path will be optimized with redex */
  static RelPath getRedexedApkPath(
      ProjectFilesystem projectFilesystem, BuildTarget buildTarget, BinaryType binaryType) {
    RelPath path =
        BuildTargetPaths.getGenPath(projectFilesystem.getBuckPaths(), buildTarget, "%s__redex");
    return path.resolveRel(buildTarget.getShortName() + ".redex." + getExtension(binaryType));
  }
  ...
  static RelPath getManifestPath(ProjectFilesystem projectFilesystem, BuildTarget buildTarget) {
    return BuildTargetPaths.getGenPath(
        projectFilesystem.getBuckPaths(), buildTarget, "%s/AndroidManifest.xml");
  }

  /** All native-libs-as-assets are copied to this directory before running apkbuilder. */
  static RelPath getPathForNativeLibsAsAssets(
      ProjectFilesystem projectFilesystem, BuildTarget buildTarget) {
  ...
  private static RelPath getPath(
      String format, ProjectFilesystem projectFilesystem, BuildTarget buildTarget) {
    return BuildTargetPaths.getGenPath(projectFilesystem.getBuckPaths(), buildTarget, format);
  }
}
