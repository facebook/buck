...
class XctestOutputParsing {
  ...
  private static final Pattern SUITE_FINISHED_PATTERN = Pattern.compile(
      "Test Suite '(.*)' (finished|passed|failed) at (.*).");
  private static final Pattern SUITE_FINISHED_DETAILS_PATTERN = Pattern.compile(
  ...
  public static class EndTestCaseEvent {
    ...
    @Nullable
    public List<TestError> exceptions = null;
  }
  ...
  private static String formatTestMessage(EndTestCaseEvent endTestCaseEvent) {
    if (endTestCaseEvent.exceptions != null &&
        !endTestCaseEvent.exceptions.isEmpty()) {
  ...
  /**
   * Decodes a stream of output lines as produced by {@code xctest} and
  ...
  public static void handleSuiteFinished(
    ...
    Matcher detailsMatcher =
        XctestOutputParsing.SUITE_FINISHED_DETAILS_PATTERN.matcher(line);
  ...
  public static void handleCaseFinished(
    ...
    LOG.debug("Finished %s: %s, duration %f",
        event.test,
        event.succeeded ? "passed" : "failed",
        event.totalDuration);
  }

  public static void handleError(
      Matcher matcher,
      ImmutableList.Builder<TestError> exceptions) {
