...
public class AppleCxxDebugSymbolLinkStrategy implements CxxDebugSymbolLinkStrategy {
  ...
  private final ImmutableSet<Path> focusedBuildOutputPaths;
  ...
  public AppleCxxDebugSymbolLinkStrategy(
      ImmutableSet<String> focusedTargets, ImmutableList<Arg> linkerArgs) {
    this.focusedBuildOutputPaths = createFocusedBuildOutputPaths(focusedTargets, linkerArgs);
    this.focusedBuildOputputPathStrings =
        focusedBuildOutputPaths.stream().map(Path::toString).collect(ImmutableSet.toImmutableSet());
  }

  @Override
  public ImmutableSet<Path> getFocusedBuildOutputPaths() {
    return focusedBuildOutputPaths;
  }

  private ImmutableSet<Path> createFocusedBuildOutputPaths(
      ImmutableSet<String> focusedTargets, ImmutableList<Arg> args) {
    ...
    Set<Path> focusedBuildOutputPaths = new HashSet<>();
    ...
    for (Arg arg : args) {
      ...
      if (sourcePath instanceof ExplicitBuildTargetSourcePath) {
        ...
        if (focusedTargets.contains(targetString)) {
          focusedBuildOutputPaths.add(explicitBuildTargetSourcePath.getResolvedPath());
        }
      }
    }
