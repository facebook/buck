...
class CommandLineBuildTargetNormalizer {
  ...
  CommandLineBuildTargetNormalizer(
    ...
    this.rootRelativePackage =
        CommandLineTargetNodeSpecParser.getRootRelativePackagePath(
            rootCell, clientWorkingDirectory);
    this.shouldRelativize =
        buckConfig.getView(CliConfig.class).getRelativizeTargetsToWorkingDirectory();
    this.normalizer =
        arg -> {
  ...
  @VisibleForTesting
  String normalizeBuildTargetIdentifier(String buildTargetFromCommandLine) {
    ...
    if (shouldRelativize) {
      target =
          CommandLineTargetNodeSpecParser.addPackagePathToRelativeBuildTarget(
              rootRelativePackage, target);
    }
