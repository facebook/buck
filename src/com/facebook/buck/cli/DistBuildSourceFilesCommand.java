...
/**
 * Distributed build debug command that prints out all files in the local hard-drive whose
 * contents were hashed in order to take part in any rule key computation. This capturing is done
 * at the level of the ActionGraph after all transformation and enhancement steps have taken place.
 * Only files in this list will take part of the distributed build and will be lazily materialised
 * on the remote BuildSlave servers.
 *
 * This command can be run in two modes:
 * 1. Produce the list of used files for a repository in the local machine passing any number
 *    of build targets.
 * 2. Fetch and output the list of targets used for a distributed build that has already occurred.
 *    This is done by passing in the [stampede-id] of the build.
 */
public class DistBuildSourceFilesCommand extends AbstractDistBuildCommand {
  ...
  /**
   * The distributed build state does not contain any cell absolute IDs or paths. It just contains
   * a name hint/alias for the cell. All cells will be rooted off whatever value is provided to
   * this option.
   */
  @Option(name = "--cells-root-path",
      usage = "Path where all cells will be rooted from.")
  ...
  /**
   * Runs the first stage of a distributed build locally to compute all required source files
   * for remote materialisation.
   */
  private void runLocally(CommandRunnerParams params) throws IOException, InterruptedException {
    try (CommandThreadManager pool = new CommandThreadManager(
        "DistBuildSourceFiles",
        getConcurrencyLimit(params.getBuckConfig()))) {
      BuildJobState jobState = BuildCommand.getDistBuildState(
          arguments,
          params,
          pool.getExecutor());
   ...
   */
  private void runUsingStampedeId(
      CommandRunnerParams params,
      StampedeId stampedeId) throws IOException {
   ...
   */
  private void outputResultToTempFile(
      CommandRunnerParams params,
      BuildJobState jobState) throws IOException {
    ...
    params.getConsole().printSuccess(
        ...
        writtenLineCount,
        outputFileAbs.toAbsolutePath().toString());
  }
}
