...
public class ProjectCommand extends AbstractCommandRunner<ProjectCommandOptions> {
  ...
  @Override
  ProjectCommandOptions createOptions() {
  ...
  int runCommandWithOptionsInternal(CommandRunnerParams params, ProjectCommandOptions options)
    ...
    if (options.getIde(params.getBuckConfig()) == ProjectCommandOptions.Ide.XCODE) {
      checkForAndKillXcodeIfRunning(params, options.getIdePrompt(params.getBuckConfig()));
    }
    ...
    TargetGraph projectGraph = projectGraphParser.buildTargetGraphForTargetNodeSpecs(
        getTargetNodeSpecsForIde(
            options.getIde(params.getBuckConfig()),
            passedInTargetsSet,
    ...
    switch (options.getIde(params.getBuckConfig())) {
      case INTELLIJ:
  ...
  int runIntellijProjectGenerator(
    ...
    try (ExecutionContext executionContext = createExecutionContext(params)) {
      Project project = new Project(
          ...
          actionGraph,
          options.getBasePathToAliasMap(params.getBuckConfig()),
          ...
          params.getRepository().getFilesystem(),
          options.getPathToDefaultAndroidManifest(params.getBuckConfig()),
          new IntellijConfig(params.getBuckConfig()),
          options.getPathToPostProcessScript(params.getBuckConfig()),
          new PythonBuckConfig(
              params.getBuckConfig(),
              new ExecutableFinder()).getPythonInterpreter(),
          params.getObjectMapper(),
          options.isAndroidAutoGenerateDisabled(params.getBuckConfig()),
      ...
      try {
        ...
        if (options.hasInitialTargets(params.getBuckConfig()) ||
            !additionalInitialTargets.isEmpty()) {
          BuildCommand buildCommand = new BuildCommand();
          BuildCommandOptions buildOptions = options.createBuildCommandOptionsWithInitialTargets(
                  params.getBuckConfig(),
  ...
  int runXcodeProjectGenerator(
    ...
    if (options.getReadOnly(params.getBuckConfig())) {
