...
if sys.platform != "darwin":
    IGNORE_PREFIXES.extend(["test/com/facebook/buck/apple"])
...
def getOwningRulesData(repo_root, test_files):
    ...
    try:
        ...
        cmd_output = run_process(
            cmd_prefix + ["targets", "--json", "--referenced-file", "@" + t_name],
            env=env,
            cwd=repo_root,
        )
...
def main():
    ...
    for b in misplaced_buck_files:
        print(
            b,
            "lives in `testdata`, you should rename it to BUCK.fixture "
