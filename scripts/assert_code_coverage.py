...
# naturally lower.
CODE_COVERAGE_GOAL = {"Linux": 78, "Darwin": 68}
...
def is_covered_class_name(class_name, generated_class_names):
    ...
    for generated_class_name in generated_class_names:
        if class_name == generated_class_name or class_name.startswith(
...
def calculate_code_coverage():
    ...
    coverage_by_package = filter(
        lambda x: x.get_number_of_children() > 0, coverage_by_package
    )
