...
class ArtifactHistory(object):
    ...
    def add_entry(self, rulekey, timestamp, event_name, info, result):
        ...
        if self.rulekey != new_entry.rulekey:
            raise Exception(
                "Attempt to insert event with rulekey="
                + rulekey
                + " into artifact history with rulekey="
                + self.rulekey
            )
    ...
    def expand(self, show_timestamp, show_event_name, show_result, show_info):
        ...
        for i, item in enumerate(self.entries):
            if print_something:
                repr += "\tItem: \n"
                repr += item.expand(
                    show_timestamp, show_event_name, show_result, show_info
                )
        return repr
...
def parseArgs():
    ...
    description = (
        "Prints out rulekeys with all attempts to get the cache item for each rulekey."
    )
...
def parse_line(line):
    ...
    event_name = buck_event_tmp_line[: buck_event_tmp_line.find("]")][
        len(EVENT_NAME_PREFIX) :
    ]
...
def analyze(log_file, show_only_misses, fields_to_show):
    ...
    for i, artifact_history_item in enumerate(rulekey_to_history.values()):
        ...
        if show_only_misses and not got_hit or not show_only_misses:
            print(
                artifact_history_item.expand(
                    TIMESTAMP_KEY in fields_to_show,
