...
def buck_build_target(args, cwd, target, perftest_side, log_as_perftest=True):
  ...
  with open(logfile_path, 'r') as logfile:
    for line in logfile.readlines():
      ...
      if match:
        cache_results[
          match.group('cache_result')].append({
...
def run_tests_for_diff(args, revisions_to_test, test_index, last_results):
  ...
  try:
    log('== Checking new buck revision for problems with absolute paths ==')
    ...
    for target, result in results.iteritems():
      if (len(result.cache_results.keys()) != 1 or
        ...
        log('Building %s at revision %s with the new buck version '
            ...
            'abosolute path.' %  (target,  revisions_to_test[test_index - 1]))
    ...
    for target, result in results.iteritems():
      if (len(result.cache_results.keys()) != 1 or
        ...
        result.cache_results.pop('DIR_HIT', None)
        raise Exception('Doing a noop build for %s at revision %s with the '
          'new buck version did not hit all of it\'s keys.\nMissed Rules:'
          ' %s' % (
            target,
...
def main():
  ...
  # Checkout the revision previous to the test and warm up the local dir cace.
  # git_clean(args.repo_under_test)
