...
class DumpTest(unittest.TestCase):
    ...
    def test_can_dump_export_map_in_json(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(os.path.join(temp_dir, 'DEFS'), 'w') as defs_file:
                defs_file.write('foo = "FOO"')
            output = self._dump('--json', '--cell_root', 'cell=' + temp_dir,
                                'export_map', build_file_path)
    ...
    def test_can_dump_export_map_as_load_functions_in_json(self):
        ...
        self.assertEqual('["load(\\"cell//:DEFS\\", \\"foo\\")"]',
                         output.strip())

    def test_can_dump_export_map_with_packages_as_load_functions_in_plain_text(
            self):
        with tempfile.TemporaryDirectory() as temp_dir:
    ...
    def test_can_dump_export_map_as_load_functions_with_cell_prefix_in_text(
            self):
        with tempfile.TemporaryDirectory() as temp_dir:
            build_file_path = os.path.join(temp_dir, 'BUCK')
            with open(build_file_path, 'w') as build_file:
    ...
    def test_can_dump_export_map_using_load_function_import_string_fmt_in_json(
            self):
        with tempfile.TemporaryDirectory() as temp_dir:
            build_file_path = os.path.join(temp_dir, 'BUCK')
            with open(build_file_path, 'w') as build_file:
            ...
            with open(os.path.join(temp_dir, 'DEFS'), 'w') as defs_file:
                defs_file.write('foo = "FOO"')
            output = self._dump(
                '--cell_root', 'cell=' + temp_dir, '--json', 'export_map',
