...
def main():
    ...
    parser.add_argument('--input-target', required=True,
                        help='Name of the target to analyze.')
    parser.add_argument('--output-repo', required=True,
                        help='Output path for generated repository.')
    parser.add_argument('--output-repo-size', type=int, required=True,
                        help='Approximate number of targets to generate.')
    args = parser.parse_args()
    args.input_target = subprocess.check_output([
        'buck', 'targets', args.input_target,
    ], cwd=args.input_repo).decode('utf8').strip('\n')
    project_data_json = subprocess.check_output([
        'buck', 'query', 'deps({})'.format(args.input_target),
        '--output-attributes', '.*',
    ], cwd=args.input_repo)
    ...
    while True:
        ...
        if count % 100 == 0:
            ...
            target_name = '//' + target['buck.base_path'] + ':' + target['name']
            targets_by_name[target_name] = target
            transitive_deps = len(get_transitive_deps(
                target_name, targets_by_name))
