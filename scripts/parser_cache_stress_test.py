...
def parseArgs():
    ...
    parser.add_argument(
        'target_to_test',
        help='Buck targets to test, i.e. //apps/myapp:')
    parser.add_argument(
        'iteration_count',
        help='Number of iterations to run')
    parser.add_argument(
        'stepping',
        help='Number of commits to jump in each iteration')
...
def invoke(repo_path, command_as_list):
    ...
    if exit_code != 0:
        sys.exit("Failed to invoke command: " + str(command_as_list) + "\nstderr:\n" + str(err))
    return out
...
def main():
    ...
    for i in range(0, int(args.iteration_count)):
        ...
        run_iteration(args.repo_root, args.buck_path, args.target_to_test, int(args.stepping))
