...
class Paths(object):
    ...
    def __init__(self, root_dir):
        ...
        self.stampede_processed_file = os.path.join(
                self.root_dir,
                'stampede.processed.log')
        self.strace_raw_file = os.path.join(self.root_dir, 'strace.raw.log')
        self.strace_processed_file = os.path.join(
                self.root_dir,
                'strace.processed.log')
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=os.path.join(
                os.path.expanduser('~'),
                'tmp/find_undeclared_source_files'),
        help='Root dir where all files will be created.',
    )
    parser.add_argument(
        'build_targets',
        nargs='+',
        help='Buck build targets to analyse.')
    parser.add_argument(
        '-m',
        '--mode',
        dest='modes',
    ...
    parser.add_argument(
        ...
        dest='includes',
        action='append',
        help=('If any include dir prefix is passed, then the processed output '
...
def run_strace(out_file, build_targets):
    ...
    buck_cmd = ('buck build {build_targets} '
                ...
                '--config cache.slb_server_pool=').format(
...
def run_stampede(out_file, build_targets):
    ...
    cmd = ('buck distbuild sourcefiles '
           ...
           '--no-cache '
           '--config cache.mode= '
           '--config cache.slb_server_pool= {build_targets}').format(
...
def process_stampede(raw_file, processed_file, includes):
    ...
    Log.success('[{}] lines were written into [{}].'.format(
            len(all_lines),
            processed_file))
...
def is_valid_path(abs_path, includes):
    ...
    #   - DEFS files.
    if '/.' in abs_path or \
            '/buck-out/' in abs_path or \
            abs_path.endswith('BUCK') or \
            abs_path.endswith('TARGETS') or \
            abs_path.endswith('DEFS'):
        return False
    if includes:
...
def confirm_at_repo_root():
    ...
    if not os.path.isfile(buckconfig_path):
        msg = ('This script must be run from the repository root.'
               ' Could not find [.buckconfig] in CWD [{}].').format(os.getcwd())
        Log.error(msg)
        raise Exception(msg)
