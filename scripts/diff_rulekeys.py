...
def parseArgs():
    description = (
        """Analyze RuleKey differences between two builds.
...
> echo '"""
        + LOGGER_NAME
        + """.level=FINER' > .bucklogging.properties
...
class KeyValueDiff(object):
    ...
    ORDER_ONLY_REMAINING = (
        "Only order of remaining entries differs: " + "[{left}] vs [{right}]."
    )
    ORDER_REPS_REMAINING = (
        "Order and repetition count of remaining "
        + "entries differs: [{left}] vs [{right}]."
    )
    ORDER_AND_CASE_ONLY = (
        "Only order and letter casing (Upper Case vs "
    ...
    def diff(self):
        ...
        if sorted(self._left) == sorted(self._right):
            return [
                KeyValueDiff.ORDER_ONLY.format(
                    left=", ".join(self._left), right=", ".join(self._right)
                )
            ]
        ...
        left_not_in_order = []
        right_not_in_order = []
        for l, r in zip_longest(left_common, right_common):
            if l == r:
                continue
            if l is not None:
        ...
        self._interesting_paths.update(
            list(filter(None, map(self._filterPathValue, left_only.union(right_only))))
        )
...
class RuleKeyStructureInfo(object):
    ...
    def getRuleKeyRefs(self, values):
        return [
            (value, RuleKeyStructureInfo._extractRuleKeyRef(value)) for value in values
        ]
        ...
        if file_path.endswith(".gz"):
            with gzip.open(file_path, "rb") as raw_log:
                ...
                utf8_log = codecs.StreamReaderWriter(
                    raw_log, info.streamreader, info.streamwriter
                )
    ...
    def _extractRuleKeyRef(value):
        ...
        def isRuleKeyRef(value):
            return value.endswith(RULE_KEY_REF_END) and value.startswith(
                RULE_KEY_REF_START
            )
                    ...
                    with open(path, "rb") as f:
                        while True:
                            ...
                            if len(buf) == 0:
                                break
                            h.update(buf)
                    result.append(" %s exists and hashes to %s" % (path, h.hexdigest()))
...
def diffValues(left_values, right_values):
    left_diff, right_diff = zip(
        *filter(lambda tup: tup[0] != tup[1], zip_longest(left_values, right_values))
    )
    return (
        filter(lambda l: l is not None, left_diff),
        filter(lambda l: l is not None, right_diff),
    )


def diffInternal(
    label, left_s, left_info, right_s, right_info, verbose, format_tuple, check_paths
):
    ...
    for key in keys:
        ...
        if key.endswith(("Deps", "deps")):
            right_names = [
                right_info.getNameForKey(right_key)
                for (_, right_key) in right_with_keys
            ]
            right_index = dict(
        ...
        both_with_keys = zip_longest(left_with_keys, right_with_keys)
...
def diff(name, left_info, right_info, verbose, format_tuple=None, check_paths=False):
    ...
    result, _ = diffAndReturnSeen(
        [(name, (left_key, right_key))],
        left_info,
        right_info,
        verbose,
        format_tuple,
    ...
    if not result and left_key != right_key:
        result.append("I don't know why RuleKeys for {} do not match.".format(name))
    return result


def diffAll(
    left_info, right_info, verbose, format_tuple=None, check_paths=False, excludes=None
):
    ...
    while True:
        ...
        if left_key is None:
            all_results.append(
                "Skipping {} because it is missing" + "from the left log.".format(name)
            )
            continue
        right_key = right_info.getKeyForName(name)
        if right_key is None:
            all_results.append(
                "Skipping {} because it is missing" + "from the right log.".format(name)
            )
            continue
...
def compute_rulekey_mismatches(
    left_info, right_info, left_name="left", right_name="right"
):
    ...
    for name in left_names.union(right_names):
        ...
        if left_key is None:
            ...
            mismatch.append(
                "{} {}:{} != {}:{}".format(
                    name, left_name, left_key, right_name, right_key
                )
            )
    return mismatch
...
def main():
    ...
    if name:
        ...
        if left_key is None:
            raise KeyError(
                (
                    "Left log does not contain {}. Did you forget "
                    + "to enable logging? (see help)."
                ).format(name)
            )
        if right_key is None:
            raise KeyError(
                (
                    "Right log does not contain {}. Did you forget "
                    + "to enable logging? (see help)."
                ).format(name)
            )
        ...
        if left_args != right_args:
            print(
                "Commands used to generate the logs are not identical: [",
                left_args,
                "] vs [",
                right_args,
