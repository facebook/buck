...
public class TestForwardRelativePath implements Comparable<TestForwardRelativePath> {
  public static final TestForwardRelativePath EMPTY = new TestForwardRelativePath(new String[0]);

  private final String[] segments;

  private TestForwardRelativePath(String[] segments) {
   ...
   */
  public static TestForwardRelativePath of(String path) {
   ...
   */
  public static TestForwardRelativePath ofSubstring(String path, int offset) {
    Preconditions.checkArgument(offset <= path.length());
    if (offset == path.length()) {
      return EMPTY;
    }

    return new TestForwardRelativePath(splitAndIntern(path, offset));
  }
  ...
  public Iterable<TestForwardRelativePath> asIterable() {
    if (segments.length == 0) {
      return ImmutableList.of(this);
    }

    return Arrays.stream(segments)
        .map(s -> new TestForwardRelativePath(new String[] {s}))
        .collect(ImmutableList.toImmutableList());
  }

  /** Similar to {@link Path#getParent()}. */
  @Nullable
  public TestForwardRelativePath getParent() {
    if (segments.length < 2) {
      return null;
    }
    return new TestForwardRelativePath(Arrays.copyOf(this.segments, segments.length - 1));
  }
   ...
   */
  public static TestForwardRelativePath ofPath(Path path) {
    ...
    if (path instanceof BuckUnixPath) {
      ...
      return new TestForwardRelativePath(segments);
    } else {
  ...
  /** Construct from given relative {@link Path}. */
  public static TestForwardRelativePath ofRelPath(RelPath path) {
    return ofPath(path.getPath());
  }

  /** Append given path to the current path */
  public TestForwardRelativePath resolve(TestForwardRelativePath other) {
    if (this.isEmpty()) {
      return other;
  ...
  public TestForwardRelativePath resolve(String other) {
  ...
  public Optional<TestForwardRelativePath> nameAsPath() {
    if (isEmpty()) {
      return Optional.empty();
    } else {
      return Optional.of(new TestForwardRelativePath(new String[] {segments[segments.length - 1]}));
    }
  }

  /** Path without last segment */
  public Optional<TestForwardRelativePath> parent() {
    if (isEmpty()) {
      return Optional.empty();
    } else if (segments.length == 1) {
      return Optional.of(EMPTY);
    } else {
      return Optional.of(new TestForwardRelativePath(Arrays.copyOf(segments, segments.length - 1)));
    }
  }
  ...
  public boolean equals(Object o) {
    ...
    TestForwardRelativePath that = (TestForwardRelativePath) o;
   ...
   */
  public boolean startsWith(TestForwardRelativePath path) {
   ...
   */
  public boolean endsWith(TestForwardRelativePath path) {
    if (this.segments.length < path.segments.length) {
      return false;
    }
   ...
   */
  public String relativize(TestForwardRelativePath other) {
    int prefix = 0;
    for (; ; ) {
  ...
  @Override
  public int compareTo(TestForwardRelativePath that) {
    for (int i = 0; ; ++i) {
