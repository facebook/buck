...
public class InterfaceValidatorTest extends CompilerTreeApiTest {
  ...
  public void testPackageReferenceFromInnerTypeSucceeds() throws IOException {
    compileWithValidation(ImmutableMap.of(
        "Foo.java",
        Joiner.on('\n').join(
  ...
  public void testStarImportedTypeFromBootClasspathFails() throws IOException {
    this.compileWithValidation(Joiner.on('\n').join(
        "import java.util.*;",
        "public abstract class Foo implements List<String> { }"));
  ...
  public void testSuperclassMemberTypeFromClasspathFails() throws IOException {
    withClasspath(ImmutableMap.of(
        ...
        Joiner.on('\n').join(
            "package com.facebook.foo;",
            "public interface Baz { interface Inner { } }")));
    compileWithValidation(ImmutableMap.of(
        "Foo.java",
        Joiner.on('\n').join(
            "package com.facebook.foo;",
    ...
    assertError(Joiner.on('\n').join(
        "Foo.java:4: error: Must qualify the name: Baz.Inner",
        "  Inner i;",
        "  ^"));
  }
  ...
  public void testQualifiedPackageMemberInnerTypeFromClasspathSucceeds() throws IOException {
    withClasspath(ImmutableMap.of(
        ...
        Joiner.on('\n').join(
            "package com.facebook.foo;",
            "class Bar { static class Inner { } }")));
    compileWithValidation(ImmutableMap.of(
        "Foo.java",
        Joiner.on('\n').join(
            "package com.facebook.foo;",
            "class Foo extends Bar.Inner { }")));
  ...
  public void testUnqualifiedPackageMemberInnerTypeFromClasspathFails() throws IOException {
    ...
    assertError(Joiner.on('\n').join(
        "Foo.java:3: error: Must qualify the name: Bar.Inner",
        "  Inner i;",
        "  ^"));
  }
