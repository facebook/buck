...
public class DryRunSpockTest {
  ...
  public void setupWorkspace() throws IOException {
    workspace = TestDataHelper.createProjectWorkspaceForScenario(
        this, "simple_spock", temporaryFolder);
    workspace.setUp();
  }
  ...
  public void dryRunShouldSucceed() throws IOException {
    // Using java_test(test_type='spock', ...), a dry run will visit all test classes
    // but disable all the test methods
    ProjectWorkspace.ProcessResult result = workspace.runBuckCommand("test", "--all", "--dry-run");
    result.assertSuccess();
    assertThat(result.getStderr(),
        containsString("0 Passed   0 Skipped   0 Failed   com.example.SimpleSpec"));
  }
}
