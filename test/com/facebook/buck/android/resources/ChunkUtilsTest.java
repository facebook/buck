...
public class ChunkUtilsTest {
  ...
  public void testFindChunksInArsc() throws Exception {
    try (ZipFile apkZip = new ZipFile(apkPath.toFile())) {
      ...
      assertEquals(
          ImmutableList.of(12, 660, 760),
          ChunkUtils.findChunks(buf, ResChunk.CHUNK_STRING_POOL)
      );

      assertEquals(
          ImmutableList.of(0),
          ChunkUtils.findChunks(buf, ResChunk.CHUNK_RESOURCE_TABLE)
      );
      assertEquals(
          ImmutableList.of(372),
          ChunkUtils.findChunks(buf, ResChunk.CHUNK_RES_TABLE_PACKAGE)
      );
      assertEquals(
      ...
      assertEquals(
          ImmutableList.of(),
          ChunkUtils.findChunks(buf, ResChunk.CHUNK_XML_TREE)
      );
      assertEquals(
          ImmutableList.of(),
          ChunkUtils.findChunks(buf, ResChunk.CHUNK_XML_REF_MAP)
      );
    }
  }
